...
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.align = 'center', fig.height = 6, fig.width = 6}
wt <- 75
ht <- 175
my_info <- data.frame(my_wt = wt, my_ht = ht/100, Category = "healthy weight")
source("myPlot.R")
g <- g + geom_point(data = my_info, aes(x = my_ht, y = my_wt, size = 2))
g <- g + geom_text(data = my_info, aes(x = my_ht, y = my_wt*1.05, label = "YOU"))
g
```
